...
def situacao_aluno(nota1, nota2, nota3, faltas, aulas_ministradas):
    ...
    presenca = (aulas_ministradas - faltas) / aulas_ministradas
...
def aumento_preco(preco):
    ...
    """
    vinte = (preco, 20, preco*0.2, round(preco*1.2, 2))
    quinze = (preco, 15, preco*0.15, round(preco*1.15, 2))
    dez = (preco, 10, preco*0.1, round(preco*1.1, 2))
    cinco = (preco, 5, preco*0.05, round(preco*1.05, 2))
    if preco <= 280:
     return vinte
...
def idade_canina(idade_humana, porte_do_cao):
    ...
    """
    pequeno = floor(idade_humana / 5)
    medio = floor(idade_humana / 6)
    grande = floor(idade_humana / 7)
...
def aumento_salario(salario):
    ...
    elif salario > 724.00 and salario <= (2*724.00):
        return round(salario*1.15, 2)
    elif salario > round(2*724.00, 2) and salario <= (5*724.00):
...
def converte_hora_24_para_12(horario):
    ...
    if horas_int <= 12:
        return str(horas_int) + ":" + str(minutos) + " am"
    else: 
        return str(horas_int - 12) + ":" + str(minutos) + " pm"
